...
infile = root_open('/uscms/home/lcadamur/nobackup/JiaFu_Ntuples_EMTF_PhaseI_12_12_2017/rateplots_mc_r305310_run2_all.0.root')
...
print "[INFO] maxEvents : ", maxEvents
...
for ievt, evt in enumerate(tree):
  if (ievt < offset): continue
  if maxEvents != -1 and ievt == (maxEvents+offset):
    break
  ...
  if full_study:
    ...
    if True:
      ...
      for itrk, trk in enumerate(evt.tracks):
        ...
        if verbose:
          print "  itrk: ", itrk, " mctruth; ", trk.mctruth
      ...
      for itrk, trk in enumerate(evt.tracks):
        if (0. <= abs(trk.eta) <= 2.5) and (trk.mode in [11,13,14,15]) and (trk.pt >= 16.):
          ...
          if cat == 'real':
            print " >>>>> Filling as real event number ", ievt, "dtheta1 ", dtheta1
...
with root_open('histos_tb_test.root', 'RECREATE') as f:
  f.mkdir('trackcounting').cd()
  for k, v in histograms.iteritems():
    v.Write()
    # if True: print "%20s" % v.GetName(), " int: ", v.Integral()
